...
/**
 * Class TickeListCrudController
 * @package App\Http\Controllers\Admin
 * @property-read \Backpack\CRUD\app\Library\CrudPanel\CrudPanel $crud
 */
class TicketListCrudController extends CrudController
{
    ...
    public function setup()
    {
        ...
        $this->crud->setRoute(config('backpack.base.route_prefix') . '/ticketlist');
        $this->crud->setEntityNameStrings('Ticket Soporte', 'Tickets Soporte');

        if (backpack_user()->can('Ver Todos Tickets')) {
            $this->crud->hasAccess('list');
        } else {
            $this->crud->denyAccess('list');
        }

        if (backpack_user()->can('Ver detalle Todos Tickets')) {
            $this->crud->hasAccess('show');
        } else {
            $this->crud->denyAccess('show');
        }

        if (backpack_user()->can('Crear Todos Tickets')) {
            $this->crud->hasAccess('create');
        } else {
            $this->crud->denyAccess('create');
        }

        if (backpack_user()->can('Editar Todos Tickets')) {
            $this->crud->hasAccess('update');
        } else {
            $this->crud->denyAccess('update');
        }

        if (backpack_user()->can('Borrar Todos Tickets')) {
    ...
    protected function addCustomCrudFilters()
    {
        ...
        $this->crud->addFilter([ // select2 filter
            ...
            return \App\Models\Estado::all()->pluck('name', 'id')->toArray();
    ...
    protected function setupUpdateOperation()
    {
        ...
        $this->crud->addfield([
            ...
            'attributes' => [
                'placeholder' => 'Este es el número de Ticket',
